...
export const MESSAGES = {
  ...
  follows: (user: User) =>
    `📺 <a href="https://stream.ru/${user.username}">${user.username}</a>`,
  resetPassword: (token: string, metadata: SessionMetadata) =>
    ...
    `<b><a href="https://stream.ru/account/recovery/${token}">Сбросить пароль</a></b>\n\n` +
  ...
  accountDeleted:
    ...
    `<b><a href="https://stream.ru/account/create">Зарегистрироваться на Stream</a></b>\n\n` +
  ...
  streamStart: (channel: User) =>
    ...
    `Смотрите здесь: <a href="https://stream.ru/${channel.username}">Перейти к трансляции</a>`,
  newFollowing: (follower: User, followersCount: number) =>
    `<b>У вас новый подписчик!</b>\n\nЭто пользователь <a href="https://stream.ru/${follower.username}">${follower.displayName}</a>\n\nИтоговое количество подписчиков на вашем канале: ${followersCount}`,
  // newSponsorship: (plan: SponsorshipPlan, sponsor: User) =>
  //     `<b>🎉 Новое спонсор!</b>\n\n` +
  //     `Вы получили новое спонсорство на план <b>${plan.title}</b>.\n` +
  //     `💰 Сумма: <b>${plan.price} ₽</b>\n` +
  //     `👤 Спонсор: <a href="https://stream.ru/${sponsor.username}">${sponsor.displayName}</a>\n` +
  //     `📅 Дата оформления: <b>${new Date().toLocaleDateString()} в ${new Date().toLocaleTimeString()}</b>`,
  // enableTwoFactor:
  //     `🔐 Обеспечьте свою безопасность!\n\n` +
  //     `Включите двухфакторную аутентификацию в <a href="https://stream.ru/dashboard/settings">настройках аккаунта</a>.`,
  // verifyChannel:
